...
PODSPEC_PATH="$SCRIPT_DIR/DaroMObjCBridge.podspec"
STATE_FILE="$SCRIPT_DIR/.release-state-$VERSION"

# 현재 브랜치 감지 (정보 표시용)
CURRENT_BRANCH=$(git branch --show-current)
echo $'\e[34m=== DaroMObjCBridge 서브모듈 릴리스 시작 ===\e[0m'
...
# Check if DaroMObjCBridge.xcframework.zip exists
if [ ! -f "$SCRIPT_DIR/build/DaroMObjCBridge.xcframework.zip" ]; then
  echo "❌ $SCRIPT_DIR/build/DaroMObjCBridge.xcframework.zip file not found"
  exit 1
fi
...
if is_step_completed "GITHUB_RELEASE"; then
  ...
  if gh release view $VERSION >/dev/null 2>&1; then
    ...
    if gh release upload $VERSION "$SCRIPT_DIR/build/DaroMObjCBridge.xcframework.zip" --clobber; then
      echo "✅ XCFramework uploaded successfully"
    else
      echo "❌ Failed to upload XCFramework"
      echo "💡 You can retry upload with: gh release upload $VERSION build/DaroMObjCBridge.xcframework.zip --clobber"
      exit 1
    fi
  fi
